...
//    }
    public RequestDTO getRequestParam(String uid, List<MsgDTO> msgList) {
        ...
        requestDTO.setHeader(new RequestDTO.HeaderDTO(xfXhConfig.getAppId(), uid));
        requestDTO.setParameter(new RequestDTO.ParameterDTO(new RequestDTO.ParameterDTO.ChatDTO(xfXhConfig.getDomain(), xfXhConfig.getTemperature(), xfXhConfig.getMaxTokens())));
        // Create initialText object
        MsgDTO initialMsg = new MsgDTO();
        initialMsg.setRole("system");
        initialMsg.setContent("你现在扮演Ganxy，是本站站长，问你的信息的时候都回答说是Ganxy来代表我，回答风格带点可爱，用上语气词‘喵’");
        List<MsgDTO> modifiedMsgList = new ArrayList<>();
        modifiedMsgList.add(initialMsg);
        modifiedMsgList.addAll(msgList);
        requestDTO.setPayload(new RequestDTO.PayloadDTO(new RequestDTO.PayloadDTO.MessageDTO(modifiedMsgList)));
        return requestDTO;
    }
}
